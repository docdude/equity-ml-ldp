...
run_name = 'wavenet_11_bce_global_zero_include_sampleweights'
...
training_parameters = {'swim_style_lr':   9.0e-6,  # Constant for swim style
                       'stroke_lr':       5.0e-6,
                       'common_lr':       1.75e-5,
                       'beta_1':          0.9,
                       'beta_2':          0.999,
...
class BalancedAdaptiveLearningRateSchedule:
    def __init__(self, initial_common_lr=2e-5, initial_style_lr=1e-4, initial_stroke_lr=1e-4,
                 adjustment_factor=1.2, tolerance=0.1, min_lr=5e-7, 
...
def train_multitask_model(model, train_gen, val_data=None, training_parameters=None, 
    ...
    def train_step(x, y_style, y_stroke, style_weights=None, stroke_weights=None):
       ...
       # with tf.GradientTape() as common_tape, \
        #     tf.GradientTape() as style_tape, \
         #    tf.GradientTape() as stroke_tape:
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Scale stroke loss to account for sequence length (180 timesteps)
            #scaled_stroke_loss = stroke_loss / tf.cast(tf.shape(y_stroke)[1], tf.float32)
            #total_common_loss = lambda_style * style_loss + lambda_stroke * scaled_stroke_loss
            total_common_loss = lambda_style * style_loss + lambda_stroke * stroke_loss

        # Calculate gradients
        common_grads = tape.gradient(total_common_loss, common_vars)
        style_grads = tape.gradient(style_loss, style_vars)

        stroke_grads = tape.gradient(stroke_loss, stroke_vars)
        del tape
        # Clip gradients
        common_grads = [tf.clip_by_value(g, -1.0, 1.0) if g is not None else g for g in common_grads]
        #common_grads = [g * 0.5 if g is not None else g for g in common_grads]  # scale down by 0.5
        ...
        # Apply gradients
        common_optimizer.apply_gradients(zip(common_grads, common_vars))
        style_optimizer.apply_gradients(zip(style_grads, style_vars))
        stroke_optimizer.apply_gradients(zip(stroke_grads, stroke_vars))
    ...
    def validate_model(x_val, y_style_val, y_stroke_val, style_weights_val=None, stroke_weights_val=None):
        ...
        style_loss = style_loss_fn(y_style_val, swim_output, sample_weight=style_weights_val)
        stroke_loss = stroke_loss_fn(y_stroke_val, stroke_output, sample_weight=stroke_weights_val)

        #style_weights_val = None
        ...
        #stroke_weights_val = stroke_weights_val
        # Weighted stroke F1
        if stroke_weights_val is not None:
...
for (i, user_test) in enumerate(users_test):
   ...
   # if data_parameters['debug']:
    ...
    folded_weights_for_user = utils.smooth_folded_weights_dict(folded_weights_for_user, method='log1p', normalize=True)
    ...
    #val_sample_weights = utils.smooth_folded_weights_array(val_sample_weights, method='log1p', normalize=True)
