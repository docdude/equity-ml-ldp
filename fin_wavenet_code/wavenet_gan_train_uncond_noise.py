...
import os
import pickle
from wavenet_gan_uncond import build_swim_discriminator, build_swim_generator
...
from utils_gan import (
    progressive_label_smoothing, smooth_positive_labels, smooth_negative_labels, spectral_loss_stft, enhanced_spectral_loss,
    compute_gradient_penalty, soft_categorical_loss, transition_focal_loss, transition_boundary_loss, zero_motion_loss, zero_style_sensor_loss, comprehensive_spectral_loss, analyze_spectral_characteristics,
...
save_path = '/teamspace/studios/this_studio/SwimmingModelPython/swim_v2/run_gan_wave64_unfilt_tanh_aug_noise3'
# Define checkpoint directory
checkpoint_dir = "/teamspace/studios/this_studio/SwimmingModelPython/swim_v2/gan_checkpoints_wave64"
...
data_parameters = {
    ...
    'window_normalization': None,  # Normalization method for windowed data
    'global_normalization': 'tanh_scaled',
    'norm_recording_range': {
    'ACC': True,   # Accelerometer data
    'GYRO': True  # Gyroscope data
    },
    'norm_recording_type': 'statistical',
...
# Normalize individual swimmer recordings

#swimming_data.normalize_recordings(detrend=None, norm_range=data_parameters['norm_recording_range'], norm_type=data_parameters['norm_recording_type'])

# Normalize dataset globally
if data_parameters['global_normalization'] == 'tanh_scaled':
    swimming_data.normalize_global_tanh(norm_range=data_parameters['norm_recording_range'])
    data_parameters['sensor_shift'] = swimming_data.sensor_shift
    data_parameters['sensor_scale'] = swimming_data.sensor_scale
    data_parameters['sensor_center'] = swimming_data.sensor_center

elif data_parameters['global_normalization'] == 'minmax':
    swimming_data.normalize_global(norm_range=data_parameters['norm_recording_range'])
    data_parameters['sensor_shift'] = swimming_data.sensor_shift
    data_parameters['sensor_scale'] = swimming_data.sensor_scale
    data_parameters['sensor_center'] = swimming_data.sensor_center
else: 
    print("No global normalization applied to dataset")
    data_parameters['sensor_shift'] = None
    data_parameters['sensor_scale'] = None
    data_parameters['sensor_center'] = None
...
generator = build_swim_generator(
    ...
    nfilt=64,
    num_styles=6,
  #  latent_dropout_rate=None,
   # latent_noise_std=None,
  #  residual_noise_std=None,
   # seed=None
)
...
if latest_ckpt:
    ...
    log_dir = os.path.join("logs", "gan_wave64", datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
def huber_per_element(y_true, y_pred, delta=0.1):
    """Custom Huber loss that returns per-element losses without reduction"""
    error = y_true - y_pred
...
def train_step6(real_data, real_styles, real_strokes,
               ...
               window_weights, freq_band_weights):


    # Generator lambda factors (precomputed from normalization + importance):
    lambda_adv = 1.0
    lambda_g_real = 100.0
    lambda_zero_rest      = 600.0   # try 50-200
    lambda_zero_matching  = 300.0    # only if you keep the second loss
  #  lambda_style = 19.37
   # lambda_stroke = 24.94
    lambda_spectral = 25.0
    lambda_temporal_sensor = 5.0
    lambda_distribution = 5.0
  ...
  #  lambda_style_real = 21.06      # Style loss for real samples
   # lambda_style_fake_disc = 17.47  # Style loss for fake samples (discriminator)
   # lambda_stroke_real = 26.60      # Stroke loss for real samples
   # lambda_stroke_fake_disc = 24.82 # Stroke loss for fake samples (discriminator)
  ...
  #  real_sensors = real_data[..., :6]
   ...
   # mae_loss = tf.keras.losses.MeanAbsoluteError(reduction=tf.keras.losses.Reduction.NONE)
    huber = tf.keras.losses.Huber(delta=0.1, reduction=tf.keras.losses.Reduction.NONE)

    with tf.GradientTape(persistent=False) as disc_tape, tf.GradientTape(persistent=True) as gen_tape:
        # Generate noise vectors
        noise_gen = tf.random.normal(shape=(batch_size, 180, 64), mean=0.0, stddev=1.0)
        # Inject more noise for style 0 to encourage variability
     #   rest_mask = tf.cast(tf.equal(real_styles, 0), tf.float32)
      #  additional_noise = tf.random.normal(tf.shape(noise_gen)) * 0.5
       # noise_gen = noise_gen + rest_mask * additional_noise

        noise_disc = tf.random.normal(shape=(batch_size, 180, 64), mean=0.0, stddev=1.0)
        # Inject more noise for style 0 to encourage variability
        #rest_mask = tf.cast(tf.equal(real_styles, 0), tf.float32)
       # additional_noise = tf.random.normal(tf.shape(noise_disc)) * 0.5
       # noise_disc = noise_disc + rest_mask * additional_noise

        # Generate fake data
        fake_data_gen = generator([noise_gen, real_styles, real_strokes], training=True)
        fake_data_disc = generator([noise_disc, real_styles, real_strokes], training=True)

        #fake_sensors_disc = fake_data_disc[..., :6]
        fake_sensors = fake_data_gen[..., :6]
        #fake_styles_gen = fake_data_gen[..., 6]
        # Get discriminator outputs for real and both fake data sets
        real_output = discriminator([real_target, real_context], training=True)
        fake_output_disc = discriminator([fake_data_disc, real_context], training=True)
        fake_output_gen = discriminator([fake_data_gen, real_context], training=True)


       # real_fake_fake_gen, style_fake_gen, stroke_fake_gen = fake_output_gen
        ...
        real_gen_labels = smooth_positive_labels(tf.ones_like(fake_output_gen))

        g_loss_adv = tf.reduce_mean(
            tf.keras.losses.binary_crossentropy(real_gen_labels, 
                                              fake_output_gen))

       # g_loss_real = tf.reduce_mean(mse_loss(fake_data_gen, real_context))
        huber_elements = huber_per_element(real_context, fake_data_gen, delta=0.1)
        g_loss_real = huber(fake_data_gen, real_context)

        # Create a mask for the sensor channels (0-5) when style is 0
        rest_mask = tf.cast(tf.equal(real_styles, 0), tf.float32)  # [B, T, 1]
        # sensor_only: zeros for rest windows, ones otherwise. Shape: [B, T, 1]
        sensor_only = 1.0 - rest_mask  # rest_mask is [B, T, 1]

        # For sensor channels (0-5), we use sensor_only (mask 0 when rest, 1 when not rest)
        # For style and stroke channels (6 and 7), we want them unmasked, i.e. ones.
        style_stroke_mask = tf.ones_like(sensor_only)  # shape [B, T, 1]

        # Now concatenate 6 copies of sensor_only and then 2 copies of style_stroke_mask along the channel axis:
        sensor_mask = tf.concat([sensor_only] * 6 + [style_stroke_mask] * 2, axis=-1)  # Resulting shape: [B, T, 8]

        # Apply the mask to the per-element Huber loss
        masked_huber = sensor_mask * huber_elements  # [B, T, 8]

        # Compute the masked average
        g_loss_real = tf.reduce_sum(masked_huber) / (tf.reduce_sum(sensor_mask) + 1e-8)
        # Additional generator losses for sensor quality (using gen fake data)
      #  spectral_loss_term = spectral_loss_orig(real_sensors, fake_sensors)
      #  spectral_loss_term = comprehensive_spectral_loss(real_sensors, fake_sensors, window_weights, freq_band_weights, norm_type='l2', scale_factor=0.1, log_compression=True)

       # temporal_sensor_loss_term = temporal_consistency_loss(fake_sensors)
    ...
    #    temporal_style_loss_term = perfect_transition_loss(real_styles, fake_styles_gen, power=2.0, perfect_weight=100.0)
     ...
     #   zero_motion_loss_term = zero_style_sensor_loss(real_sensors, fake_sensors, real_styles)
        zero_energy_loss  = zero_motion_loss(real_styles, fake_sensors)
        zero_match_loss   = zero_style_sensor_loss(real_sensors, fake_sensors, real_styles)
       ...
       # gradient_penalty = compute_gradient_penalty(real_data, fake_data_disc, discriminator)
        ...
        total_disc_loss = (
         ...
         #   lambda_style_real        * style_loss_real +
          #  lambda_style_fake_disc   * style_loss_fake_disc +
           # lambda_stroke_real       * stroke_loss_real +
            #lambda_stroke_fake_disc  * stroke_loss_fake_disc 
        ...
        total_gen_loss = (
            ...
            lambda_zero_rest   * zero_energy_loss +
            lambda_zero_matching * zero_match_loss
         #   lambda_style       * style_loss_fake_gen +
          #  lambda_stroke      * stroke_loss_fake_gen +
    ...
    """
        # scaled individual losses
        debug_adv  = lambda_adv       * g_loss_adv
        debug_real = lambda_g_real    * g_loss_real_masked
        debug_zero = lambda_zero_rest * zero_energy_loss
    # --- Debugging: compute gradient norms with the SAME persistent gen_tape ---

    grads_adv  = gen_tape.gradient(debug_adv,  generator.trainable_variables)
    grads_real = gen_tape.gradient(debug_real, generator.trainable_variables)
    grads_zero = gen_tape.gradient(debug_zero, generator.trainable_variables)


    # filter out Nones (in practice should be none)
    grads_adv  = [g for g in grads_adv  if g is not None]
    grads_real = [g for g in grads_real if g is not None]
    grads_zero = [g for g in grads_zero if g is not None]

    norm_adv  = tf.linalg.global_norm(grads_adv)  if grads_adv  else tf.constant(0.0)
    norm_real = tf.linalg.global_norm(grads_real) if grads_real else tf.constant(0.0)
    norm_zero = tf.linalg.global_norm(grads_zero) if grads_zero else tf.constant(0.0)

  #  tf.print("Gradient Norms → Adv:", norm_adv,
   #          "Reconst:", norm_real,
    #         "Zero:",   norm_zero)
    """
    ...
    #disc_grads = [g * 2.0 if g is not None else g for g in disc_grads]  # scale down by 0.5
    ...
    return {
        ...
        "gen_zero_motion": zero_energy_loss,
        "gen_zero_match": zero_match_loss
   #     'style_real': style_loss_real,
    #    'style_fake_gen': style_loss_fake_gen,
     #   'style_fake_disc': style_loss_fake_disc,
      #  'stroke_real': stroke_loss_real,
       # 'stroke_fake_gen': stroke_loss_fake_gen,
   ...
   #     'temporal_sensor': temporal_sensor_loss_term,
    #    'distribution': distribution_loss_term
       # 'temporal_style': temporal_style_loss_term
        ...
        # Add the gradient norms
    #    'grad_norm_adv': norm_adv,
     #   'grad_norm_real': norm_real,
      #  'grad_norm_zero': norm_zero
    }
...
def train_gan(generator, discriminator, data_generator, epochs, steps_per_epoch, 
    ...
    history = {
        'disc_loss': [], 'gen_loss': [], 
        'disc_real': [], 'disc_fake': [],
        'gen_adv': [], 'gen_real': [], 'gen_zero_motion': [], 'gen_zero_match': [],
        ...
        try:
            ...
            for step in range(steps_per_epoch):
               ...
               # real_styles_onehot = tf.one_hot(real_styles_3d[..., 0], depth=6)  
                ...
                real_sensors_reshape = real_sensors.reshape(-1, 6)

                results, window_weights, freq_band_weights = analyze_spectral_characteristics(real_sensors_reshape)
                ...
                metrics = train_step6(
                    ...
                    window_weights, freq_band_weights['freq_weights_array']
                )
                ...
                if step % 10 == 0:
                   ...
                   # print(f"Gradient Norms → Adv = {metrics['grad_norm_adv']:.4f}, Reconst = {metrics['grad_norm_real']:.4f}, Zero = {metrics['grad_norm_zero']:.4f}")
            ...
            with summary_writer.as_default():
                ...
                #    swim_style_similarity_match_cos = compute_swim_style_similarity_match_cos(real_styles, fake_styles)
                  ...
                  #  sensor_rmse = compute_rmse(real_sensors, fake_sensors)
                    ...
                    fd_results = compute_frechet_distance(real_sensors, fake_sensors, per_channel=True)
             ...
             #       tf.summary.scalar("Similarity/Swim_StyleMatch_Cos", swim_style_similarity_match_cos, step=epoch)
                  ...
                  #  tf.summary.scalar("Similarity/SensorsJs", sensor_similarity_js, step=epoch)
                   # tf.summary.scalar("Similarity/Sensors_RMSE", sensor_rmse, step=epoch)
                    ...
                    tf.summary.scalar("FD/Sensor Overall FD", data=fd_results['overall_fd'], step=epoch)
                    tf.summary.scalar("FD/Sensor Avg Channel FD", data=fd_results['average_channel_fd'], step=epoch)
                    for channel, fd_value in fd_results['channel_fd'].items():
                        tf.summary.scalar(f'FD/Channel/{channel}', fd_value, step=epoch)
                   ...
                   #     f"Swim Style SimilarityMatchCos={swim_style_similarity_match_cos:.2f}% | "
                     ...
                     #   f"Sensor SimilarityJs={sensor_similarity_js:.2f}% | "
                      ...
                      #  f"Sensor RMSE={sensor_rmse:.2f} | "
                        ...
                        f"Overall FD={fd_results['overall_fd']:.4f} | " 
                      #  f"Sensor Maximum Mean Discrepancy={sensor_mmd_score:.4f}")
                        #f"Overall Similarity={overall_similarity:.2f}%")
                        f"Average Channel FD: {fd_results['average_channel_fd']:.4f} |")

                        # Print per-channel results
                    for channel, fd_value in fd_results['channel_fd'].items():
                        print(f"FD: {channel}: {fd_value:.4f}")
                    ...
                    # Only log if we actually got a figure back
                    if fig_rest is not None:
                        img = plot_to_image(fig_rest)
                        tf.summary.image("Rest Sample Comparison", img, step=epoch)
                        plt.close(fig_rest)
                    else:
                        # Optional: print a warning once
                        tf.print("⚠️ No valid rest‐style samples; skipping Rest Sample Comparison image.")
...
def validate_generator(generator, x_val, y_val_raw, y_stroke_val):
   ...
   # val_noise = tf.random.normal([len(x_val), 180, 32])    
    val_noise = tf.random.normal(shape=(len(x_val), 180, 64), mean=0.0, stddev=1.0)
    y_val_raw_3d = np.expand_dims(y_val_raw, axis=-1)
  ...
  #  rest_mask = tf.cast(tf.equal(val_styles, 0), tf.float32)
   # additional_noise = tf.random.normal(tf.shape(val_noise)) * 0.5
    #val_noise = val_noise + rest_mask * additional_noise
