...
data_parameters = {'users':                users,   # Users whose data is loaded
                   ...
                   'validation_set': {
                        0: 2,  # Transition (NULL + TURN combined as 0)
                        1: 2,  # Freestyle
                        2: 2,  # Breaststroke
                        3: 2,  # Backstroke
                        4: 2,  # Butterfly
...
for (i, user_test) in enumerate(users_test):
    ...
    log_dir = f"logs/wave/{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}_run-{user_test}-{run_name}"
  ...
  # if data_parameters['debug']:
   ...
   # optimizer = tf.keras.optimizers.Adam(learning_rate=training_parameters['lr'], beta_1=training_parameters['beta_1'],
    ...
    hyperparameters_path = f'/teamspace/studios/this_studio/SwimmingModelPython/swim_v2/best_hyperparameters/{run_name}/best_hyperparameters_1.pkl'

    print(f"Training model with best hyperparameters from: {hyperparameters_path}")
    model = lstm_dual_model.train_with_best_hyperparameters(
   ...
   # batch_data, batch_outputs, batch_sample_weights = next(gen)
    ...
    if training_parameters['swim_style_output'] and training_parameters['stroke_label_output']:
        validation_data = (x_val, {'swim_style_output': y_val_cat, 'stroke_label_output': y_stroke_val},
                                {'swim_style_output': final_swim_weights, 'stroke_label_output': (val_stroke_mask 
                                    if training_parameters['stroke_mask'] 
                                    else final_stroke_weights)})
