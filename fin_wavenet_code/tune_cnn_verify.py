...
def validate_hyperparameters(tuner, input_shape, training_parameters, data_parameters):
    for trial_id in range(tuner.oracle.max_trials):
        hp_config = tuner.oracle.populate_space(trial_id)["values"]  # Get hyperparameter set
        
        try:
            print(f"\n🔍 Validating Trial {trial_id} \n")#with params:\n{hp_config}")

            # Manually create and populate the HyperParameters object
            hp = kt.HyperParameters()
            for key, value in hp_config.items():
                hp.Fixed(key, value)  # Assign fixed values to prevent errors
            ...
            print(f"\n❌ Trial {trial_id}: ERROR - {e}")
            print("⚠️ Possible cause: One of the following hyperparameters:")
            for key, value in hp_config.items():
                print(f"  {key}: {value}")
...
def run_hyperparameter_tuning(input_shape, data_parameters, training_parameters):
    # Create the hypermodel
    hypermodel = SwimStrokeHyperModel(input_shape, training_parameters, None, data_parameters)
    ...
    validate_hyperparameters(tuner, input_shape, training_parameters, data_parameters)
    """       
...
if __name__ == '__main__':
    ...
    run_hyperparameter_tuning(input_shape, data_parameters, training_parameters)
