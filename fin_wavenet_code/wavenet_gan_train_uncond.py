...
from utils_gan import (
    ...
    compute_gradient_penalty, soft_categorical_loss, transition_focal_loss, transition_boundary_loss, zero_style_sensor_loss, comprehensive_spectral_loss, analyze_spectral_characteristics,
...
save_path = '/teamspace/studios/this_studio/SwimmingModelPython/swim_v2/run_gan_wave64_unfilt_tanh_aug_zeronone_v3'
...
data_parameters = {
    ...
    'global_normalization_per_label': True,
...
if data_parameters['global_normalization'] == 'tanh_scaled':
    swimming_data.normalize_global_tanh_per_label(norm_range=data_parameters['norm_recording_range'],per_label_axis=data_parameters['global_normalization_per_label'])
...
discriminator = build_swim_discriminator(
    time_steps=180,
    nfilt=64#len(data_parameters['labels']),
...
def train_step6(real_data, real_styles, real_strokes,
  ...
  #  real_sensors = real_data[..., :6]
     ...
     #   rest_mask = tf.cast(tf.equal(real_styles, 0), tf.float32)
      #  additional_noise = tf.random.normal(tf.shape(noise_gen)) * 0.5
       ...
       # real_fake_fake_gen, style_fake_gen, stroke_fake_gen = fake_output_gen
        ...
        #g_loss_real = tf.reduce_mean(mae_loss(fake_data_gen, real_context))
        g_loss_real = tf.reduce_mean(huber(fake_data_gen, real_context))
    ...
    return {
      ...
      #  "gen_zero": zero_motion_loss_term
