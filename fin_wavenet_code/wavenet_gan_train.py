...
import os
import pickle
from wavenet_gan import build_swim_discriminator, build_swim_generator
...
save_path = '/teamspace/studios/this_studio/SwimmingModelPython/swim_v2/run_gan_wave_full'
...
def train_step6(real_data,
  ...
  #  lambda_style = 19.37
   ...
   # lambda_temporal_style = 15
    ...
    lambda_d_real = 1.10          # Adversarial loss for real samples
    lambda_d_fake = 1.24         # Adversarial loss for fake samples
  ...
  #  real_sensors = real_data[..., :6]
    ...
    with tf.GradientTape(persistent=False) as disc_tape, tf.GradientTape(persistent=False) as gen_tape:
        ...
        # Generate fake data
        fake_data_gen = generator([noise, real_context], training=True)
       ...
       # fake_data_gen = fake_data_disc
        ...
        fake_output = discriminator([fake_data_gen, real_context], training=True)
       ...
       # real_fake_fake_gen, style_fake_gen, stroke_fake_gen = fake_output_gen
        ...
        fake_labels = smooth_negative_labels(tf.zeros_like(fake_output))
        ...
        g_loss_adv = tf.reduce_mean(
            tf.keras.losses.binary_crossentropy(tf.ones_like(real_output), 
                                              fake_output))
...
def train_gan(generator, discriminator, data_generator, epochs, steps_per_epoch, 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        try:
            ...
            if epoch % 10 == 0:
                val_fake_data = validate_generator(generator, x_val)                
...
def validate_generator(generator, x_val):
   ...
   # val_noise = tf.random.normal([len(x_val), 180, 32])    
    ...
    real_data = tf.convert_to_tensor(x_val_combined, tf.float32) # real_data_combined = np.concatenate((real_sensors, real_styles_3d, real_strokes), axis=2, dtype=np.float32)

    val_fake_data = generator([val_noise, real_data], training=False)
